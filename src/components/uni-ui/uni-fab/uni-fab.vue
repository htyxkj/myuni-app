...
export default class uniFab extends Vue {
	@Prop({ type: Object, default: {} }) pattern!: object;
	@Prop({ type: String, default: 'left' }) horizontal!: string;
	@Prop({ type: String, default: 'bottom' }) vertical!: string;
	@Prop({ type: String, default: 'vertical' }) direction!: string;
	@Prop({ type: Array, default: [] }) content!: Array<any>;
	top: number = 0;
	fabShow: boolean = false;
	flug: boolean = true;
	showContent: boolean = false;
	styles: any = { color: '#3c3e49', selectedColor: '#007AFF', backgroundColor: '#fff', buttonColor: '#3c3e49' };
	created() {
		if (this.top === 0) {
			this.fabShow = true;
		}
	 ...
	 * 获取 位置信息
	 */
	getPosition(types: number, paramA: string, paramB?: string) {
		if (types === 0) {
			...
			return this.showContent && this.direction === paramA ? this.contentWidth : this.contentWidthMin;
		}
	}

	@Watch('pattern')
	patternChange(newValue: any, oldValue: any) {
	...
	get flexDirectionStart() {
		return this.getPosition(1, 'vertical', 'top');
	}
	get flexDirectionEnd() {
