...
<script lang="ts">
	...
	export default class mescrollUni extends Vue{
		mescroll:any = null
		viewId:string = 'id_' + Math.random().toString(36).substr(2)
		downHight:number = 0
		downRotate:number = 0
		downText:string = ''
		...
		@Prop({type:Object}) up!:object // 上拉加载的参数配置
		...
		created() {
			...
			vm.mescroll.resetScrollTo((y:number, t:number) => {
				...
				if (t === 0) {
					...
					vm.scrollAnim = true;
					vm.mescroll.getStep(curY, y, (step:any) => { // 此写法可支持配置t
						vm.scrollTop = step
					}, t)
				}
			})
